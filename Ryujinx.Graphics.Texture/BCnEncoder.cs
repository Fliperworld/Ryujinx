...
namespace Ryujinx.Graphics.Texture
{
    public static class BCnEncoder
    {
        ...
        public static byte[] EncodeBC1(byte[] data, int width, int height, int depth, int levels, int layers)
        {
            int size = 0;
            ...
            for (int l = 0; l < levels; l++)
            {
                ...
                size += w * h * 8 * Math.Max(1, depth >> l) * layers;
            }
                ...
                for (int l2 = 0; l2 < layers; l2++)
                {
                    for (int z = 0; z < depth; z++)
                    {
                        ...
                        output.AsMemory().Slice(imageBaseOOffs),
                            ...
                            width,
                            height,
                            EncodeMode.Multithreaded);

                        imageBaseIOffs += width * height * 4;
                        imageBaseOOffs += w * h * 8;
                    }
                }
